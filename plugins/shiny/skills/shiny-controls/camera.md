...
`RequestBarcodesAsync(ct)` is the same gate but completes with **every** code in that frame. `RequestDocumentImageAsync(requireNewDocument: true, ct)` waits for the previously delivered page to leave the frame first, so a loop collecting pages does not capture the same one repeatedly. A restart (lens flip, analyzer swap) re-arms any outstanding request instead of leaving it waiting forever. The preview asks `getUserMedia` for an *ideal* 1920×1080, because without it Chrome opens at 640×480. For a modal camera with no page of your own, use the Blazor `IMediaService` (see media-service.md).
