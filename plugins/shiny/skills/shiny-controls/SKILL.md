...
- **Gantt** (MAUI + Blazor): `GanttView` — a project timeline; task grid on the left, scrolling time axis on the right, draggable bars. Hierarchy (flat `ParentId` **or** nested `Children`, mixable) with summary rollups weighted by duration, milestones, baselines, deadlines, constraints, all four dependency types with lag (negative lag = lead time), working-time calendars, and a critical path. **The entire scheduling engine lives in `Shiny.Controls.Gantt.Shared`** (`GanttModel`/`GanttCalendar`/`GanttScheduler`/`GanttGeometry`/`GanttLayoutMetrics`) and is shared verbatim — including bar rectangles and arrow routes — so neither control does date arithmetic; generate model work against `Shiny.Controls.Gantt`, not the view. Items must be `GanttTask` (no generic `TItem`); put your own object in `GanttTask.Item`. Every drag produces a cancellable `GanttSchedulePlan` listing the edited task plus every cascaded one, and the plan reverts itself — undo is a stack of them. `CascadeMode` defaults to `PushOnly` (successors move later, never earlier). `ShowCriticalPath` and `AllowDependencyEdit` are both **off** by default. On Blazor the component fills its parent and needs a bounded height. See gantt.md

- **Kanban** (MAUI + Blazor): `KanbanView` — a board of columns you drag cards between, with WIP limits, swimlanes, collapsible columns and an inline add-card composer. **Every decision lives in `KanbanBoard`** (bucketing, WIP counts, drop verdicts, move planning, order renumbering) and the file is mirrored verbatim into both packages — a test fails if the copies drift — so generate model work against `KanbanBoard`, not the view. Cards must be `KanbanCard` (no generic `TItem`); put your own object in `KanbanCard.Item`. Colours on cards, columns, swimlanes and labels are **strings** (`"#2563eb"`), not platform colours. A `WipLimit` counts the whole column across every swimlane, and reordering inside a column is always allowed however full it is; `ShowWipLimits` (styling) and `WipBehavior` (`None`/`Warn` default/`Block`) are independent. Every move raises cancellable `CardMoving` with the full `KanbanMove` — which records the source column, swimlane and index, so undo is a stack of them — then `CardMoved`; a refused drop raises `DropRejected` with a reason. `AddCardMode` is **off** by default and the board **never creates a card**: handle `AddCardRequested` and add to your own collection. `SwimlaneMode` defaults to `None`, in which case `SwimlaneId` is ignored. MAUI drags on `PanGestureRecognizer` (never `DragGestureRecognizer`); Blazor drags on pointer events (never HTML5 DnD) and needs a bounded height. See kanban.md

- **Diagram** (MAUI + Blazor): `DiagramView` — a bindable graph of shapes and connections: org charts, decision trees, flowcharts, mindmaps. **Add-on packages** (`Shiny.Maui.Controls.Diagram` / `Shiny.Blazor.Controls.Diagram`), never core. **Not the Mermaid control** — that renders a picture from mermaid *text*; this is a graph you bind to and edit, so pick it when the source of truth is a collection of nodes. The whole engine is `Shiny.Controls.Diagram.Shared` (namespace `Shiny.Controls.Diagramming`): `DiagramModel`, the five layouts, `ConnectionRouter`, `ShapeGeometry` and `DiagramHitTester`, shared verbatim so neither control computes geometry; generate model work against that namespace, not the view. Items must be `DiagramNode`/`DiagramConnection` (no generic `TItem`); put your own object in `DiagramNode.Item`. Hierarchy is flat `ParentId` **or** nested `Children`, mixable — and a hierarchy with **no connections still draws its links**, so an org chart is nested nodes plus an empty connection list. Layouts are `Tree` (default; arranges a hierarchy, so a graph that rejoins loses an edge), `Layered` (Sugiyama — reach for this the moment the graph rejoins or loops), `MindMap`, `Radial`, `ForceDirected` and `None`; all deterministic. The property is **`LayoutKind`, never `Layout`** on both hosts — on MAUI `Layout` hides `VisualElement.Layout(Rect)`. Connectors are `Orthogonal` (default), `Straight` or `Bezier`, anchored to the **real shape outline** rather than the bounding box, with `Auto` ports snapping to the layout's axis. **Editing is off by default** — `AllowNodeDrag`, `AllowConnectionEdit`, `AllowMultiSelect`, `AllowDelete` (Blazor only; MAUI uses `DeleteSelection()`); `AllowPan`/`AllowZoom`/`AllowSelection` are on. Dragging a node sets `IsPinned`, which holds it through a re-layout. Every gesture produces a cancellable `DiagramEditPlan` listing the whole cascade, and the plan reverts itself — `Undo()`/`Redo()` are built in. Validation reports and never throws: read `DiagramModel.Issues` from `Built`/`OnBuilt`. `DiagramJson.Save`/`Load` round-trips the graph. On Blazor the component fills its parent and needs a bounded height. See diagram.md
- **Gamepad** (MAUI iOS/Android + Blazor): `GamepadView` — an on-screen game controller, as an overlay over a game (default `Sizing="Anchored"`, touches that miss every element pass through) or inline (`Sizing="Uniform"`). **Add-on packages** (`Shiny.Maui.Controls.Gamepad` / `Shiny.Blazor.Controls.Gamepad`); engine `Shiny.Controls.Gamepad.Shared` (namespace `Shiny.Controls.Gaming`). It is a real Shiny.Gamepad `IGamepad` (`GamepadKind.Virtual`) — read `view.Gamepad.GetState()` or the button/axis events. Always emit `UseShinyGamepad()` (MAUI) / `AddShinyGamepad()` (Blazor) **instead of** `AddGamepads()`. Buttons are positional (`A` = bottom face button whatever the label). MAUI properties are `ControllerLayout` and `ControllerScale`, never `Layout`/`Scale`. Haptics are two switches: `ButtonHapticFeedback` (on) and `DirectionalHapticFeedback` (off). See gamepad.md

- **Floor Plan** (MAUI + Blazor): `FloorPlanView` — rooms, walls, doors, cubicles, outlets, furniture and custom SVG stencils on a pan/zoom SkiaSharp surface. **Add-on packages** (`Shiny.Maui.Controls.FloorPlan` / `Shiny.Blazor.Controls.FloorPlan`), never core — the engine is SkiaSharp and core carries none. MAUI needs `UseShinyFloorPlan()` or the canvas has no platform view and the page is blank; XAML namespace is `http://shiny.net/maui/floorplan`, not the core `shiny:` one. Blazor needs a **sized container** — a drawn surface has no intrinsic height. Two modes and they behave like different controls: `Edit` gives the `ActiveTool` the pointer and puts eight resize handles on the selection; `View` makes the whole surface a pan surface, blocks every edit, and raises `ElementTapped`/`OnElementTapped` on a tap that did not turn into a drag (the seating-chart case — no toolbar needed). The whole engine is `Shiny.Controls.FloorPlan.Shared` (namespace `Shiny.Controls.FloorPlan`): `FloorPlanEngine`, the document model, renderers, `FloorPlanHitTester`, the camera and the tools; generate document and tool work against that namespace, not the view. Sizes are **plan units**, not pixels. Element `Style.FillColor`/`StrokeColor` are **nullable and default to null**, meaning "take the theme" — that is what makes one saved plan read correctly in light and dark; set them only when the plan really is colour-coded. Tools hold gesture state, so build a **new** tool instance each time rather than reusing one. `PlaceElementTool` takes either `Kind` + companions (XAML/parameter friendly) or a `Factory` that overrides everything. Viewport operations (`ZoomToFit`/`ZoomIn`/`ZoomOut`/`ScrollTo`) are **methods, not bindable properties** — they need the surface size. `net10.0-macos` (AppKit) is not supported unless the app also references `Shiny.Maui.Controls.Office`, which ships the missing Skia canvas handler. See floorplan.md
...
- **FloatingToolbar**: A toolbar that floats over a *control* the way a tooltip does — icons, labels, badges, dropdown menus and an overflow, **horizontal or vertical**. Shares the tooltip's anchoring (`AnchorTriggerBinder` + `TooltipPlacementSolver` on MAUI, `tooltip.js` on Blazor). One instance serves many controls: `FloatingToolbar.AttachTo` per row on MAUI, a multi-match selector on Blazor. Item type is `ShinyToolbarItem` on MAUI (MAUI already has a `ToolbarItem`), `ToolbarItem` on Blazor. Not the docked page bar — that is `ShinyToolbar`. See floating-toolbar.md
...
- **ButtonGroup** (both hosts, **core**): Joins related buttons into one segmented control - inner corners square off and adjacent outlines collapse into one edge, horizontally or vertically. A container for *actions* first; `SelectionMode` (`None`/`Single`/`Multiple`, default **None**) is opt-in and turns the same markup into a segmented picker with a two-way `SelectedIndex`, `SelectedIndexes`, `AllowDeselect` and `SelectionChanged`. While a selection mode is on the group owns each segment's `Appearance` (`SelectedAppearance` Filled / `UnselectedAppearance` Outlined) - **except** where the segment set its own, which wins as the unselected look. Segments are `ShinyButton`s plus `ButtonGroupSeparator` (a hairline for a split button, for two *filled* segments only) and `ButtonGroupText` (a static label/unit, never selectable); a nested `ButtonGroup` makes the outer one a spaced *cluster*. MAUI is a `StackLayout` that pushes squared corners onto each segment and collapses outlined edges with a negative margin; Blazor does the joining in CSS over `:first-child`/`:last-child` with logical corner properties, so RTL mirrors for free, and adds `aria-pressed` on selection segments. Reach for `ShinyTabBar` when the segments switch *pages*. See button-group.md
- **TagEntry** (both hosts, **core**): A free-text tags field - type and press Enter (or a delimiter) to commit each value as a removable chip in one bordered box. Pasting "a, b, c" commits three tags while *typing* the same characters leaves "c" in the editor; `Delimiters` defaults to `[","]` and an **empty** list means Enter-only, which is how a tag gets to contain a comma. `MaxTags` caps the count and leaves the typed text alone, duplicates are dropped quietly (`AllowDuplicates`, `CaseSensitiveDuplicates`), and validation is the cancellable `TagAdding` on MAUI / `TagValidator` on Blazor. Backspace on empty text removes the newest chip - on MAUI that is a zero-width-space sentinel, because there is no portable key-down on an `Entry`, switchable off with `BackspaceRemovesTag`. `ChipTemplate`/`ChipContent` replaces a chip's label but never its remove button. **Not** a suggestion list: when the values come from a known list, use `AutoCompleteEntry`. See tag-entry.md
- **ChipGroup** (both hosts, **core**): A bound set of chips - one per item in `ItemsSource`, selectable singly or in any number, and the selection is **the items themselves** rather than an index, which is the whole difference from `ButtonGroup` (markup segments, `SelectedIndex`). `SelectionMode` defaults to **Single** here (the opposite of ButtonGroup's `None`), with `Multiple` for filter chips and `None` for action chips that only report `ChipTapped`. Two-way `SelectedItem` and `SelectedItems` are both kept accurate in every mode, and MAUI writes *into* the bound `SelectedItems` list rather than replacing it. `AllowDeselect` (off) lets a second tap clear a single-selection group; `MaxSelectionCount` refuses the new chip rather than dropping the oldest pick; a selection the source stops offering is dropped. Chips show `ToString()` unless given `ItemDisplayBinding`/`DisplayMemberPath` (MAUI) or `DisplaySelector` (Blazor), and `ItemTemplate`/`ChipContent` replaces the label but never the check or the ✕. `AllowRemove` is off by default and takes the item out of the source when the source is a writable list, with the cancellable `ChipRemoving` in front of it. Chips wrap by default; `Wrap="False"` needs a horizontal `ScrollView` around it on MAUI and scrolls by itself on Blazor. Blazor is generic - `ChipGroup<TItem>`. See chip-group.md
...
- **ProgressBar**: A progress bar with gradient fill and a Vista-style shimmer pulse that sweeps left-to-right. Configurable `PulseLength` (width of sheen) and `PulseSpeed` (sweep duration). Triggers on value change or timed interval. Supports indeterminate mode and text overlay. `Segments` (with `SegmentSpacing`) splits the bar into separate steps instead of a solid bar - the fill runs across them, so line `Maximum` up with the count for whole steps. The fill **slides** to each new value rather than snapping, symmetrically - a value that drops drains back at the same rate it filled - via `AnimateProgress`/`ProgressAnimationDuration`/`ProgressAnimationEasing`; only a value change animates, while a width change driven by layout snaps. See progressbar.md
...
- **ShinyFlexLayout** (**MAUI only**, core package - Blazor uses native CSS flexbox): a fast replacement for MAUI's `FlexLayout` with the same properties and `Microsoft.Maui.Layouts` enums (`Direction`, `Wrap`, `JustifyContent`, `AlignItems`, `AlignContent`; attached `Grow`/`Shrink`/`Basis`/`AlignSelf`/`Order` under the `shiny:ShinyFlexLayout.` prefix), plus `RowSpacing`/`ColumnSpacing` gaps (CSS row-gap/column-gap). Child measurements are cached until the child invalidates, so same-size passes measure nothing, a single change re-measures one child, and resizes reuse leaf measurements. Shrink is CSS-weighted; `Position` (absolute) is not supported. Prefer it over `FlexLayout` in generated MAUI code. See flex-layout.md

- **YogaLayout** (**MAUI + Blazor**, core packages): the layout model of [Yoga](https://www.yogalayout.dev/) (Meta's React Native flexbox engine; credit to the Yoga project), implemented independently: pure C# on MAUI, CSS with Yoga's defaults on Blazor. Yoga defaults: Column, shrink 0, align-content FlexStart, min size 0. Extras: absolute `PositionType` with `Left/Top/Right/Bottom/Start/End` insets, percentage lengths (`"50%"`), `AspectRatio`, `AutoMargins` (`YogaEdges`), `Display`. MAUI child props are attached; sizes are `NodeWidth`/`NodeHeight` (never `YogaLayout.Width`, which collides with `VisualElement.Width`). On Blazor every node is a nested `<YogaLayout>` and lengths are strings. See yoga-layout.md.
...
- Build an indented tree/hierarchical *list* (file browser, folder picker, category tree) — TreeView
- Draw an org chart, decision tree, flowchart or mindmap as boxes and connecting lines — DiagramView,
  not TreeView: an org chart drawn as an indented list is a list, not a chart
