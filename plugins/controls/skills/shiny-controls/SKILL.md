---
name: shiny-controls
description: Generate UI for .NET MAUI (Shiny.Maui.Controls) and Blazor (Shiny.Blazor.Controls). Includes TableView, TreeView (lazy load, drag/drop reorder), FloatingPanel/OverlayHost/ShinyContentPage, DurationPicker, FrostedGlassView, Toast service, PillView, BadgeView, ImageViewer (pinch/pan zoom), ImageEditor (crop/rotate/draw), ChatView with MessageTemplate, SecurityPin, Fab/FabMenu, Scheduler (calendar/agenda), Markdown controls, Barcodes & QR codes (13 symbologies via ZXing), CameraView with pluggable IFrameAnalyzer (Barcode/Face/Motion/OCR/Documents including invoices, driver's licenses, health/credit cards, passport MRZ), AutoCompleteEntry, CountryPicker, AddressEntry (geocoding), SignaturePad, TextEntry (floating label), gradient Slider, gradient ProgressBar with shimmer, ParallaxCollectionView/ParallaxList, Overlay/LoadingOverlay, SkeletonView, Desktop add-on (TrayIcon, Docking, on-screen keyboard), Blazor Kiosk add-on, extensible IFeedbackService (haptic/TTS/sound) and UseFeedback across controls.
auto_invoke: true
triggers:
  - tableview
  - table view
